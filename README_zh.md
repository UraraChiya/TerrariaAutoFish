...
* **自动持久化**: 所有配置（包括过滤列表和运行模式）都会实时保存至 `config.ini`。
* **强力内存扫描**: 采用动态模式匹配技术定位游戏指针，无需维护硬编码地址，兼容性更强。

---

## 🚀 使用方法
抛竿后自动开始，请保持光标位置不变，程序会模拟鼠标点击。
### GUI 图形界面模式 (推荐)
```sh
uv run main.py
```
...
```
> **注意**: 控制台模式无法读取 config.ini，请手动修改源码设置过滤和模式。
...
> **注意**: 当选择“全部捕获”模式时，过滤设置按钮将自动变为灰色禁用状态。

---

## 🛠️ 环境要求

* **游戏版本**: 已在 **Terraria 1.4.5.3** (单人模式和专用服务器) 测试通过。
